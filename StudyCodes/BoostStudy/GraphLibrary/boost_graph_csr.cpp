...
int main() {
    ...
    // using zip, make pairs, which is edge list
    graph_t graph{construct_inplace_from_sources_and_targets, sources, targets, n_vertices};

    std::vector<vertex_data_t> vertex_data{{15},
                                           {-5},
                                           {10}};

    auto vertex_data_map = make_iterator_property_map(vertex_data.begin(), boost::get(vertex_index, graph));

    auto priority = [&vertex_data_map](auto vertex) { return boost::get(vertex_data_map, vertex).priority; };
